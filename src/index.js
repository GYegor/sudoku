module.exports = function solveSudoku(matrix) {
  ...
  function searchSolve(arr) {
    let matrixFix = copyMatrix(arr);
    all: for (let kk = 1; kk < 9; kk++) {
      for (let oo = 0; oo < 9; oo++) {
        if (Array.isArray(matrixFix[kk][oo])) {
          let pp;
          for (pp = 0; pp < matrixFix[kk][oo].length; pp++) {
  ...
  function copyMatrix(arra) {
    if (!(arra instanceof Object)) {
      return arra;
    }
    let i;
    let Constructor = arra.constructor;
    let clone = new Constructor();
    for (i in arra) {
      clone[i] = copyMatrix(arra[i]);
    }
    return clone;
  }
